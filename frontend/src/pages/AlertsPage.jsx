...
const METRICS = [
  ...
  { value: 'vwc',        label: 'VWC (humedad suelo)', unit: '%' },
  { value: 'battery',    label: 'Batería',       unit: 'V' },
  { value: 'connection', label: 'Conexión',      unit: '' },
];
...
export default function AlertsPage() {
  ...
  const [runToast, setRunToast] = useState(null); // { alertasGeneradas: number } | null
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {/* Comprobar ahora + Demo email — solo superadmin */}
          {isAdmin && (
            <>
              <button
...
const DEFAULT_FORM = {
  source: 'fieldclimate',
  stationId: '',
  metric: '',   // set dynamically from station metrics
  operator: 'gt',
  threshold: '',
};
...
function ConfigModal({ open, onClose, onSave }) {
  ...
  // Métricas reales de la estación seleccionada
  ...
  // Auto-selecciona la primera métrica cuando se cargan
  useEffect(() => {
