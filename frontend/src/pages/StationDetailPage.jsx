...
import { generarDescripcion } from '@/utils/estaciones.helpers';
...
export default function StationDetailPage() {
    ...
    queryFn: () =>
      isFC
        ? stationsApi.getFCHistory(id, from, to).then((r) => {
            ...
            // LOG TEMPORAL — eliminar tras verificar
  ...
  // Auto-detect coordinates from station data
  const autoCoords = (() => {
  ...
  // Métricas disponibles y color asignado por índice (igual que las Line)
  ...
  // Al cargar nuevos datos, preseleccionar temperatura + humedad (o todo si no hay)
  useEffect(() => {
  ...
  const toggleMetric = (key) =>
    setSelectedMetrics(prev => {
      ...
      if (next.has(key)) { if (next.size > 1) next.delete(key); } // mínimo 1 activa
      else next.add(key);
      return next;
    });
...
function LiveDataGrid({ data, source }) {
  ...
  // FC y Cesens usan el mismo formato: { principal: [...], detalle: [...], fecha }
...
// Igual que el backend: name_original → name.original → name (string)
...
function fcTsToDate(ts) {
  ...
  const n = Number(ts);
  if (!isNaN(n) && n > 1e9) return new Date(n * 1000); // Unix segundos
  const d = new Date(ts);                               // ISO string u otro
...
function parseFCChart(histData) {
  ...
  // Preprocesar sensores: extraer nombre, elegir agregación, filtrar principales
  ...
  // Fallback: si ningún sensor pasa el filtro principal, mostrar todos
  ...
  return fechas
    .map((ts, i) => {
      ...
      if (Object.keys(punto).length === 0) return null; // sin datos en este tick
