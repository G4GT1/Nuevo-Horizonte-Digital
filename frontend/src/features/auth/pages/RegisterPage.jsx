...
/**
 * Hook que crea el schema de validacion del formulario de registro.
 * @returns {import('zod').ZodObject} Schema con nombre, apellidos, email y password
 */
...
/**
 * Calcula la fortaleza de una contrasena en una escala del 0 al 5.
 * Suma puntos por: longitud >= 8, longitud >= 12, mayuscula, numero y caracter especial.
 * @param {string} pwd - Contrasena a evaluar
 * @returns {number} Puntuacion de 0 (vacia) a 5 (muy fuerte)
 */
...
/** Etiquetas de fortaleza por indice (0=sin texto, 1=muy debil ... 5=fuerte) */
const strengthLabel = ['', 'Muy débil', 'Débil', 'Regular', 'Buena', 'Fuerte'];
/** Clases de color de la barra de fortaleza por indice */
const strengthColor = ['', 'bg-danger', 'bg-warn', 'bg-yellow-400', 'bg-success', 'bg-success'];

/**
 * Pagina de registro de nuevos usuarios.
 * Tras registrarse correctamente muestra una pantalla de exito pidiendo verificar el email.
 * @component
 * @returns {JSX.Element}
 */
export default function RegisterPage() {
  ...
  /* shakeKey se incrementa para re-lanzar la animacion de sacudida en cada error */
  ...
  /* Observa el campo password en tiempo real para calcular la fortaleza */
  const password = watch('password', '');
  const strength = passwordStrength(password);

  /**
   * Envia los datos de registro al backend.
   * Si el email ya existe muestra el error especifico; si hay otros errores los concatena.
  ...
  /**
   * Devuelve clases CSS del input segun si tiene o no error de validacion.
  ...
  /* Pantalla de exito: se muestra en lugar del formulario tras registrar correctamente */
  if (success) {
    return (
      <motion.div
  ...
  return (
    <div className="space-y-6">
      ...
      {/* key={shakeKey} re-monta el motion.form para re-lanzar la animacion de sacudida */}
      <motion.form
        key={shakeKey}
      ...
      >
        ...
        <div>
          ...
          {/* Barra de fortaleza: solo se muestra cuando el usuario escribe algo */}
          {password.length > 0 && (
            <div className="mt-1.5 space-y-1">
              <div className="flex gap-1">
                {/* 5 segmentos: los rellenos con color hasta el nivel de fortaleza */}
                {[1, 2, 3, 4, 5].map((i) => (
                  <div
                    key={i}
